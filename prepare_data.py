...
class PrepareData:
    ...
    def decompose_train_paths( self, paths, dictionary, file_pos, file_names ):
        """
        Decompose the paths to variable length sub-paths while moving the starting tool
        """
        sub_paths_pos = list()
        ...
        for index, item in enumerate( paths ):
            ...
            if len_tools <= self.max_tool_sequence_len:
                for start_pos in range( 0, len_tools ):
                    sequence = tools[ start_pos: window_size + start_pos + 1 ]
    ...
    def get_data_labels_mat( self ):
        ...
        train_paths = self.decompose_train_paths( train_paths, dictionary, self.train_file, self.train_file_sequence )
